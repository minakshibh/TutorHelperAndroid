...
public class AddLessonActivity extends Activity implements AsyncResponseForTutorHelper{
	...
	private String str_starttime="",str_endtime="",str_day="",str_tutorid="",str_setdate="",str_setdate2="",str_setdate3=""
			,str_setdate4="",str_setdate5="",str_setdate6="",str_setdate1="",str_setdate0;
	private TextView txt_sun,txt_mon,txt_tus,txt_wed,txt_thur,txt_sat,txt_fri;
	...
	private DateFormat durationformatter = new SimpleDateFormat("HH:mm"); 
	...
	private LinearLayout lay_endtime_edit,lay_starttime_edit,lay_Lessondate,lay_Lessonenddate;
	private LinearLayout  lay_sun,lay_mon,lay_tue,lay_wed,lay_thu,lay_sat,lay_fri,lay_all; 
	...
	private ArrayList<Date> arraylist_dateformate;
	...
	private void initialiselayout() {
		...
		arraylist_dateformate=new ArrayList<Date>();
		...
		//lay_Lessondate,lay_Lessonenddate;
...
//check box
	...
	private void onClickListeners() {
		...
		public void onClick(View v) {
			...
			{
				Intent intent=new Intent(AddLessonActivity.this,TutorDashboard.class);
...
//for start time.............
	                    	...
	                    	minute= mintuecheck(minute);    	
	                        	 ...
	                        	 Date date = null,date1=null;
									try {
										...
										str_starttime = timeformatter.format(date);
										String endtime = timeformatter.format(date1);
				        		    	btn_editst.setText(str_starttime);
	                        			 ...
	                        			 String currentdate = timeformatter.format(c.getTime());
	 			        			     ...
	 			        			     else 
	 			        			     {
	 			        			    	if(datecheck==1)
	 			        			    	{
	 			        			    		...
	 			        			    		Util.alertMessage(AddLessonActivity.this, "Please select Start time Greater from current time");  
     ...
     //check for time........ 	 
                	...
                	if(timecheck2==1){
                		...
                		Util.alertMessage(AddLessonActivity.this, "Please select end time Greater from lesson start time");  
            			btn_editet.setText("select end time"); 
            			timecheck2=0;
                	}
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					if(isChecked)
					{
						...
						if(str_isrec.equalsIgnoreCase("no"))
						{
							...
							String bigdate=""+arraylistdate.get(arraylistdate.size() - 1);
							Date date=null;
							try {
								DateFormat dtimeformatter = new SimpleDateFormat("yyyyMMdd");  
...
//check for start time......  	 
           			 ...
           			 String currenttime = timeformatter.format(ctime.getTime());
        			     ...
        			     else 
        			     {
        			    	if(datecheck==1)
        			    	{
        			    		...
        			    		System.err.println("Please select Start time Greater from current time");	
        			    		Util.alertMessage(AddLessonActivity.this,"Time "+btn_editst.getText().toString() + " is passed away. Please select future time.");  
        			    		btn_editst.setText("select start time"); 
        			    			}
        			     	}
			     ...
			     {
			    	Util.alertMessage(AddLessonActivity.this, "Please select lesson start date Greater from current date");  
...
// for lesson end date
	   ...
	   DatePickerDialog dpd = new DatePickerDialog(AddLessonActivity.this, new DatePickerDialog.OnDateSetListener() {
    		  ...
    		  if( datecheck2==1)
    		  {
    			  Util.alertMessage(AddLessonActivity.this, "Please select lesson end date Greater from lesson start date");  
		    	 ed_Lessonenddate.setText("select end date");
		    	datecheck2=0;
    		  }
               }
...
/*	 private void findDatetoweekday()
		{
		 if(check)
		}*/
	...
	{
/*		long milliseconds_startlesson = 0,
				milliseconds_endlesson=0,
				get_milliseconds_startlesson=0,
				get_milliseconds_endtlesson=0;
		Date getstartdate,getenddate,startdate,enddate;*/
		ArrayList<Lesson_Booked> array_lesson=new ArrayList<Lesson_Booked>();
			int EndA=Integer.parseInt(btn_editst.getText().toString().trim().replace(":", "")+"00");
			int EndB=Integer.parseInt(btn_editet.getText().toString().trim().replace(":", "")+"00");
				...
				try {
					...
					//milliseconds_startlesson = date1.getTime();
					
					date2 = new SimpleDateFormat("yyyy-MM-dd").parse(lesson_enddate);
					//milliseconds_endlesson = date2.getTime();
					//date1= (Date)formatter.parse(lesson_startdate);
					//date2= (Date)formatter.parse(lesson_enddate);
			...
			array_lesson=dbHandler.getLessonBooked("adll", date3,date4);
				...
				{
					System.err.println("get lesson"+array_lesson.get(i).getDate().replace(":", ""));
					
					
					int StartA=Integer.parseInt(array_lesson.get(i).getStart_timing().replace(":", ""));
					int StartB=Integer.parseInt(array_lesson.get(i).getEnd_timing().replace(":", ""));//start b
				...
				if(StartA < EndB)
				{
					if(EndA < StartB)
					{
						//Util.alertMessage(AddLessonActivity.this, "this date already booked");
				...
				///////		 	 
				/*Date l_starttime = null,l_endtime=null,getdatabase_Starttime=null,getdatabase_Endtime=null;
			 	 
			 	String lesson_starttime=btn_editst.getText().toString().trim();
				String lesson_endtime=btn_editet.getText().toString().trim();
				
				
				try {
						DateFormat dtimeformatter = new SimpleDateFormat("HH:ss");  
						l_starttime = (Date)dtimeformatter.parse(lesson_starttime);
						l_endtime = (Date)dtimeformatter.parse(lesson_endtime);
						...
						/*String get_startlesson=array_lesson.get(i).getDate();//+array_lesson.get(i).getStart_timing();
						Date date5 = new SimpleDateFormat("yyyy-MM-ddHH:mm").parse(get_startlesson);
						get_milliseconds_startlesson = date5.getTime();
						System.err.println("sl"+get_milliseconds_startlesson);
						
						String get_endtlesson=array_lesson.get(i).getDate()+array_lesson.get(i).getEnd_timing();
						Date date6 = new SimpleDateFormat("yyyy-MM-ddHH:mm").parse(get_endtlesson);
						get_milliseconds_endtlesson = date6.getTime();
						System.err.println("el"+get_milliseconds_endtlesson);*/
						
						
						/*String get_startlesson=array_lesson.get(i).getDate();//+array_lesson.get(i).getStart_timing();
						getstartdate = new SimpleDateFormat("yyyy-MM-dd").parse(get_startlesson);
						//get_milliseconds_startlesson = date5.getTime();
						System.err.println("sl"+get_milliseconds_startlesson);*/
						
					/*	if(getstartdate.after(date)|| milliseconds_startlesson<get_milliseconds_endtlesson)
							{
							if(milliseconds_endlesson>get_milliseconds_startlesson || milliseconds_endlesson<get_milliseconds_endtlesson)
							{
								//Util.alertMessage(AddLessonActivity.this, "date overlap");
								date_overlap=1;
								break;
								}
							else
							{
								date_overlap=0;
										System.err.println("not over lap");
							}
								}
						else
						{
					...
					/*if(.equalsIgnoreCase(array_lesson.get(i).getDate())
	...
	public void Add_Lesson()
	{
		...
		if(date_overlap==1)
		{
			Util.alertMessage(AddLessonActivity.this, "Leasson "+lesson_des+ " already Exist for this time slot.");
			date_overlap=0;	
			}
		else
		{
		...
		Util.alertMessage(AddLessonActivity.this,"Please check your internet connection");
	}
		}
...
/*	private void getnextTime() {
	...
	public void findDuration()
	{
		...
		Date date2 = null;
		try {
			date1 = simpleDateFormat.parse(btn_editst.getText().toString().trim());
			 date2 = simpleDateFormat.parse(btn_editet.getText().toString().trim());
		} catch (ParseException e) {
		...
		min =(int) difference/(60*1000);
		  ...
		  str_duration=""+min; //  int minutes = (int)interval / 60; // interval minus hours part (in seconds) divided by 60 yields minutes
...
//			 	 Date date = null;
//				try {
//					DateFormat dtimeformatter = new SimpleDateFormat("m");  
//					date = (Date)dtimeformatter.parse(str_dur);
//					
//					//timeformatter = new SimpleDateFormat("HH:mm:ss");
//					str_duration = durationformatter.format(date);
//				} catch (ParseException e) {
//					e.printStackTrace();
//				}
//		  }
//		  else  if(str_dur.length()==2)
//		  {
//			 	 Date date = null;
//				try {
//					DateFormat dtimeformatter = new SimpleDateFormat("mm");  
//					date = (Date)dtimeformatter.parse(str_dur);
//					
//					//timeformatter = new SimpleDateFormat("HH:mm:ss");
//					str_duration = durationformatter.format(date);
...
//				}
//		  }
//		 else  if(str_dur.length()==3)
//				  {
//					 	 Date date = null;
//						try {
//							DateFormat dtimeformatter = new SimpleDateFormat("hmm");  
//							date = (Date)dtimeformatter.parse(str_dur);
//							
//							//timeformatter = new SimpleDateFormat("HH:mm:ss");
//							str_duration = durationformatter.format(date);
...
//			 	 Date date = null;
//				try {
//					DateFormat dtimeformatter = new SimpleDateFormat("hhmm");  
//					date = (Date)dtimeformatter.parse(str_dur);
//					
//					//timeformatter = new SimpleDateFormat("HH:mm:ss");
//					str_duration = durationformatter.format(date);
		...
		if(methodName.equals("create-lesson"))
		{
			try {
				...
				public void onClick(DialogInterface arg0, int arg1) {
					if(tutorPrefs.getString("mode", "").equalsIgnoreCase("parent"))
					{
						Intent intent=new Intent(AddLessonActivity.this,ParentDashBoard.class);
						startActivity(intent);
						for(int i = 0;i<AddActivities.size(); i++){
