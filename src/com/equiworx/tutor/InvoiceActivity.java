...
public class InvoiceActivity extends Activity {
	...
	private static Font smallBold = new Font(Font.FontFamily.TIMES_ROMAN, 12,
			Font.BOLD);
	private static Font smalls = new Font(Font.FontFamily.TIMES_ROMAN, 12,
			Font.NORMAL);
	...
	private static void addTitlePage(Document document)
		...
		preface.add(new Paragraph(
				Payment_FulllessonDetialsActivity.description, catFont));
		// preface.setAlignment(Element.ALIGN_RIGHT);
		addEmptyLine(preface, 1);
		preface.add(new Paragraph("Report Generate Date : "
				+ getCurrentDateTime()));
		preface.add(new Paragraph("Start Date : "
				+ Payment_FulllessonDetialsActivity.sdate + ""));
		preface.add(new Paragraph("End Date : "
				+ Payment_FulllessonDetialsActivity.edate));
		preface.add(new Paragraph("Days : "
				+ Payment_FulllessonDetialsActivity.getdays));
		addEmptyLine(preface, 1);
		// preface.add(new
		// Paragraph("Parent Name : "+StatementActivity.parentname+"                                      "+"Tutor Name : "+tutorname,
		// subFont));
	...
	private static void createTable(Paragraph subCatPart)
		...
		// c1 = new PdfPCell(new
		// Paragraph(teacherToolsVO.getToolName(),font10));
		table.addCell(c1);
		...
		for (int i = 0; i < Payment_LessonsDetailsActivity.arraylist_studentlist
				.size(); i++) {
			int j = i + 1;
			...
			insertCell(table,
					Payment_LessonsDetailsActivity.arraylist_studentlist.get(i)
							.getStudentId(), Element.ALIGN_CENTER, 1, smalls);
			...
			insertCell(table,
					Payment_LessonsDetailsActivity.arraylist_studentlist.get(i)
							.getName(), Element.ALIGN_CENTER, 1, smalls);
	...
	private static void createTable2(Paragraph subCatPart)
		...
		for (int i = 0; i < Payment_FulllessonDetialsActivity.array_sessiondate
				.size(); i++) {
			int j = i + 1;
			table.setHeaderRows(i);
			// table.addCell("     "+j+"");
			insertCell(table, "" + j, Element.ALIGN_CENTER, 1, smalls);
			insertCell(table,
					Payment_FulllessonDetialsActivity.array_sessiondate.get(i),
					Element.ALIGN_CENTER, 1, smalls);
			insertCell(table, "$" + Payment_FulllessonDetialsActivity.totalfess
					+ "/hr", Element.ALIGN_CENTER, 1, smalls);
	...
	public void displaypdf() {
		...
		if (file.exists()) {
			...
			Toast.makeText(getApplicationContext(), "File path is incorrect.",
					Toast.LENGTH_LONG).show();
	}
