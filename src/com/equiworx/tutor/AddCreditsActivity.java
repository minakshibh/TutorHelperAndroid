...
public class AddCreditsActivity extends Activity implements AsyncResponseForTutorHelper, OnClickListener,OnItemSelectedListener{
	...
	private void getParentId() {
		...
		//stockArr = arraylist_student.toArray(stockArr);
		 ...
		 /*Displaying Array elements*/
			...
			ArrayAdapter<String> adapter_state = new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item, stockArr);
    ...
    private void getTutorDetails(Boolean value) {
		...
		if (Util.isNetworkAvailable(AddCreditsActivity.this)){
			...
			if(payment.equals("") || remark.equals("")||select_parentId.getSelectedItemPosition()==0)
			{
				
				Util.alertMessage(AddCreditsActivity.this, "Please fill in required fields.");
			}
			else{
			try {
				payment=fees_exit.getText().toString();
			...
			if (makepayment.equalsIgnoreCase("AddCredit")) {
				...
				nameValuePairs.add(new BasicNameValuePair("parent_id", parentIdd));
				nameValuePairs.add(new BasicNameValuePair("payment_mode", "AddCredit"));
				nameValuePairs.add(new BasicNameValuePair("amount", encode_payment));
				nameValuePairs.add(new BasicNameValuePair("remarks", en_remark));
			}else if (makepayment.equalsIgnoreCase("PayFees")) {
				
			    nameValuePairs = new ArrayList<NameValuePair>();
				nameValuePairs.add(new BasicNameValuePair("tutor_id", tutorPrefs.getString("tutorID", "")));
				nameValuePairs.add(new BasicNameValuePair("parent_id", parentIdd));
				nameValuePairs.add(new BasicNameValuePair("payment_mode", "PayFees"));
				nameValuePairs.add(new BasicNameValuePair("amount", encode_payment));
				nameValuePairs.add(new BasicNameValuePair("remarks", en_remark));
			}
				
			AsyncTaskForTutorHelper mLogin = new AsyncTaskForTutorHelper(AddCreditsActivity.this, "pay-fee", nameValuePairs, value, "Please wait...");
			...
			{
			Util.alertMessage(AddCreditsActivity.this,"Please check your internet connection");
		}
	...
	public void processFinish(String output, String methodName) {
		// TODO Auto-generated method stub
		Log.d("","xxxxxxxxxxxxxxx"+output);
		if (methodName.equals("pay-fee")) {
			dialog = parser.getAddCreditResults(output);
			
			if (dialog.get(0).getSuccess().equalsIgnoreCase("success")) {
				//Intent intent=new Intent(AddCreditsActivity.this,Payment_Activity.class);
				//startActivity(intent);
				finish();
				
			}else{
				Toast.makeText(AddCreditsActivity.this,"Credits is not successfull", Toast.LENGTH_LONG).show();
