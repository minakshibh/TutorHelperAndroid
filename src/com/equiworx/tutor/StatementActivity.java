...
public class StatementActivity extends Activity implements
		AsyncResponseForTutorHelper {

	private TextView title, txt_startdate, txt_enddate;
	private RelativeLayout back;
	private int mYear, mMonth, mDay;
	private int sYear, sMonth, sDay;
	private Spinner parentSpinner;
	private LinearLayout lay_startdate, lay_enddate;
	private String str_startdate = "", parentId = "";
	private int datecheck2;
	static String startdate = "";
	static String enddate = "";
	static String parentname = "";

	private Button button_genrate;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * ArrayAdapter<Parent> spinnerArrayAdapter = new
		 * ArrayAdapter<Parent>(this, android.R.layout.simple_spinner_item,
		 * parentList);
	...
	private void getExistingParents() {
		if (Util.isNetworkAvailable(StatementActivity.this)) {
			...
			AsyncTaskForTutorHelper mLogin = new AsyncTaskForTutorHelper(
					StatementActivity.this, "fetch-parent", nameValuePairs,
					true, "Please wait...");
			mLogin.delegate = (AsyncResponseForTutorHelper) StatementActivity.this;
			mLogin.execute();
		} else {
			Util.alertMessage(StatementActivity.this,
	...
	private void PDF_Data() {
		...
		if (Util.isNetworkAvailable(StatementActivity.this)) {
			...
			nameValuePairs.add(new BasicNameValuePair("start_date",
					txt_startdate.getText().toString()));
			nameValuePairs.add(new BasicNameValuePair("end_date", txt_enddate
					.getText().toString()));
			...
			AsyncTaskForTutorHelper mLogin = new AsyncTaskForTutorHelper(
					StatementActivity.this, "parent-statement", nameValuePairs,
					true, "Please wait...");
			mLogin.delegate = (AsyncResponseForTutorHelper) StatementActivity.this;
			mLogin.execute();
		} else {
	...
	private void onclickListener() {
		...
		button_genrate.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if (txt_enddate.getText().toString()
						.equalsIgnoreCase("Select end date")
						|| txt_startdate.getText().toString()
								.equalsIgnoreCase("Select start date")) {
					Util.alertMessage(StatementActivity.this,
							"Please select date");
				} else if (parentSpinner.getSelectedItem().toString().trim()
						.equalsIgnoreCase("Select existing parent")) {
					Util.alertMessage(StatementActivity.this,
							...
							public void onDateSet(DatePicker view, int year,
								...
								try {
									...
									if (curdate.before(seldate)) {
										...
										Util.alertMessage(
												StatementActivity.this,
												"Please select start date less from current date");
										txt_startdate
										...
										// Util.alertMessage(StatementActivity.this,
										// "Please select start date Greater from current date");
										// txt_startdate.setText("select start date");
										// txt_enddate.setText("select end date");
										txt_enddate.setText("select end date");
									}
						...
						new DatePickerDialog.OnDateSetListener() {
							public void onDateSet(DatePicker view, int year,
								...
								try {
									...
									Calendar c = Calendar.getInstance();
									// String currentdate =
									// formatter.format(str_startdate);
										...
										try {
											...
											System.out
													.println("Current date => "
															+ c1.getTime());
											String currentdate = formatter
													.format(c1.getTime());
											Date seldate1 = formatter
													.parse(str_startdate);
											System.out
													.println("Select date => "
															+ seldate1);
											Date curdate1 = formatter
													.parse(currentdate);
											if (curdate1.before(seldate)) {
												...
												Util.alertMessage(
														StatementActivity.this,
														"Please select end date less from current date");
												// txt_startdate.setText("select start date");
												txt_enddate
												...
												try {
													...
													if (curtime.before(seltime)) {
														...
														Util.alertMessage(
																StatementActivity.this,
																"Please select end date less from current time");
								...
								if (datecheck2 == 1) {
									Util.alertMessage(StatementActivity.this,
											"Please select  end date Greater from  start date");
									txt_enddate.setText("select end date");
									// txt_startdate.setText("select start date");
	...
	public void processFinish(String output, String methodName) {
		...
		if (methodName.equals("fetch-parent")) {
			...
			Intent i = new Intent(StatementActivity.this,
					PdfGenerateActivity.class);
			startActivity(i);
		}
