...
public class CaldroidGridAdapter extends BaseAdapter {
	...
	public DateTime getToday() {
		...
		//	System.err.println("class==="+stoday);
			editor.putString("today",stoday);
			editor.commit();*/
	...
	protected void customizeTextView(int position, TextView cellView) {
		...
		cellView.setHeight(8);
		cellView.setWidth(8);
		cellView.setGravity(Gravity.CENTER);
		//cellView.setBackgroundColor(R.color.white);

		// Get dateTime of this cell
		DateTime dateTime = this.datetimeList.get(position);

		// Set color of the dates in previous / next month
		//set color to disable date
		if (dateTime.getMonth() != month) {
			cellView.setTextColor(resources.getColor(R.color.caldroid_lighter_gray));		}
		...
		if (shouldResetDiabledView && shouldResetSelectedView) {
			// Customize for today
			if (dateTime.equals(getToday())) {
				//cellView.setBackgroundResource(R.drawable.circle_green);// set the today background
			} else {
				cellView.setBackgroundResource(R.color.white);
			}
		}
